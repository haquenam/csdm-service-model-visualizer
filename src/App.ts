...
const parseCsvFile = (file: File, setRecords: (records: CsdmRecord[]) => void, setError: (error: string) => void) => {
  Papa.parse(file, {
    header: true,
    skipEmptyLines: true,
...
const App = () => {
  const [records, setRecords] = useState(SAMPLE_RECORDS) as [CsdmRecord[], (records: CsdmRecord[]) => void];
  ...
  const handleUpload = (event: Event) => {
    ...
    if (file) parseCsvFile(file, setRecords, setError);
  };
    ...
    h('section', { className: 'mx-auto max-w-7xl px-6 py-8 lg:px-8', key: 'content' }, [
      ...
      h('div', { className: 'mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm', key: 'tablewrap' }, [
        ...
        h('div', { className: 'overflow-x-auto', key: 'scroll' },
          h('table', { className: 'min-w-full divide-y divide-slate-200 text-sm' }, [
